...
for instance in controller-0 controller-1 controller-2; do
  ...
  vagrant ssh ${instance} -c "sudo mkdir -p /etc/kubernetes/config && cd /home/vagrant && sudo chown root: kube-apiserver kube-controller-manager kube-scheduler kubectl && sudo chmod +x kube-apiserver kube-controller-manager kube-scheduler kubectl && sudo cp kube-apiserver kube-controller-manager kube-scheduler kubectl /usr/local/bin/. && chown -R root: /usr/local/bin/*"
done
